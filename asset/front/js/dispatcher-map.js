			...
			if( result ) {
				if(result.status != 'SEARCHING') {
					if( result.status == 'ACCEPTED' || result.status == 'STARTED' ) {
						$('#wait-modal .modal-title').html('Owner Accepted The Request');
					}
	...
	if( data.length > 0 ) {
		...
		//alert(data[i].status);	
			...
			if( data[i].status == 'COMPLETED' || data[i].status == 'ACCEPTED' || data[i].status == 'STARTED' || data[i].status == 'ARRIVED' || data[i].status == 'PICKEDUP' || data[i].status == 'SCHEDULED' || data[i].status == 'PENDING' ) {
    			html += 				'<h6 class="media-heading">Owner Name: ' + data[i].provider.first_name +'</h6>';
...
function LiveTrackingInitMap() {
        ...
        div.innerHTML = '<img src="' + mapIcons['active'] + '"> ' + 'Available Owner';
...
function ajaxMapData() {
	...
	$.ajax({
		...
		dataType: "JSON",
		headers: {'X-CSRF-TOKEN': window.Laravel.csrfToken,'ongoing':ongoing,'owner':owner },
		type: "GET",
		success: function(data) {
			ajaxMarkers = data;
		}
	...
	$.get(bases_url+'/dispatcher/provider_zones',{}, function( result ) {
		if( result.length ) {
			...
			result.forEach(function( zone, index ) {
				...
				html+=		'<div class="owner_list zone_'+zone['id']+'" data-zone_id="'+zone['id']+'"  id="zone_'+zone['id']+'">';	
							if( zone['owners'].length ) {
								zone['owners'].forEach(function( owner, ind ) {
									var bg =  ( owner.provider_status == 'riding' ) ? 'bg-danger' : '';
									
				html+=					'<div class="owner  owner_'+owner.id+'  '+bg+'" id="position_'+owner.owner_position+'">';
				html+=						'<div class="vehicle_no">'+owner.service_number+'</div>';
				html+=						'<div class="timeOut">In Time:'+owner.enter_time+'</div>';
				html+=					'</div>';
								});
