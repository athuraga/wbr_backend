@extends('layouts.app',['activePage' => 'vehicle_discount'])
...
@section('content')
    ...
    <section class="section">
        ...
        <div class="section-body">
            <h2 class="section-title">{{__("Vehicle Discount Management")}}</h2>
            <p class="section-lead">{{__('Add & Manage Discounts')}}</p>
            <div class="card">
                <div class="card-header">
                    <div class="w-100">
                        @can('vehicle_discount_add')
                            <a href="{{ url('vehicle/vehicle_discount/create/') }}" class="btn btn-primary float-right">{{__('Add New')}}</a>
                        @endcan
                    </div>
                </div>
                <div class="card-body">
                                        ...
                                        @if(Gate::check('vehicle_discount_edit') || Gate::check('vehicle_discount_delete'))
                                            <td>
                                                @can('vehicle_discount_edit')
                                                    <a href="{{ url('vehicle/vehicle_discount/'.$discount->id.'/edit') }}" class="btn btn-primary btn-action mr-1" data-toggle="tooltip" title="" data-original-title="Edit"><i class="fas fa-pencil-alt"></i></a>
                                                @endcan
