...
@section('title','Edit Fleet operator')

@section('content')

<section class="section">
    <div class="section-header">
        <h1>{{__('Edit fleet operator')}}</h1>
        <div class="section-header-breadcrumb">
            @if(Auth::user()->load('roles')->roles->contains('title', 'admin'))
                <div class="breadcrumb-item active"><a href="{{ url('admin/home') }}">{{__('Dashboard')}}</a></div>
                <div class="breadcrumb-item"><a href="{{ url('admin/delivery_person') }}">{{__('Fleet operator')}}</a></div>
                <div class="breadcrumb-item">{{__('Edit a Fleet operator')}}</div>
            @endif
            @if(Auth::user()->load('roles')->roles->contains('title', 'vehicle'))
                ...
                <div class="breadcrumb-item active"><a href="{{ url('vehicle/deliveryPerson') }}">{{ $delivery_person->first_name .' - '. $delivery_person->last_name }}</a></div>
                <div class="breadcrumb-item">{{__('Edit a Fleet operator')}}</div>
            @endif
        </div>
    </div>
    ...
    <div class="section-body">
        ...
        <p class="section-lead">{{__('Edit fleet operator')}}</p>
        <form class="container-fuild" action="{{ url('admin/delivery_person/'.$delivery_person->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="card">
                <div class="card-header">
                    <h6 class="c-grey-900">{{__('Fleet operator personal information')}}</h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 mb-5">
                            <label for="Promo code name">{{__('Fleet operator image')}}</label>
                ...
                <div class="card-body">
                    <div class="mT-30">
                        ...
                        <div class="text-center">
                            <button type="submit" class="btn btn-primary" type="submit">{{__('Update Fleet operator')}}</button>
                        </div>
                    </div>
                </div>
            </div>
