...
<section class="section">
    ...
    <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
            @if(Auth::user()->load('roles')->roles->contains('title', 'admin'))
                ...
                <div class="breadcrumb-item">{{__('Fleet operator')}}</div>
            @endif
    ...
    <div class="section-body">
        <h2 class="section-title">{{__('fleet operator management')}}</h2>
        <p class="section-lead">{{__('Add, Edit, Manage Fleet operator.')}}</p>
        <div class="card">
            <div class="card-header">
                <div class="w-100">
                    @can('delivery_person_add')
                        ...
                        @endif
                        @if(Auth::user()->load('roles')->roles->contains('title', 'vehicle'))
                            <a href="{{ url('vehicle/deliveryPerson/create') }}" class="btn btn-primary float-right">{{__('Add New')}}</a>
                        @endif
                    @endcan
                </div>
            </div>
                    ...
                    <table class="table table-striped text-center" id="datatable">
                        <thead>
                            <tr>
                                ...
                                <th>#</th>
                                <th>{{__('Fleet operator profile')}}</th>
                                ...
                                @if(Gate::check('delivery_person_edit') || Gate::check('delivery_person_delete'))
                                    <td class="d-flex justify-content-center">
                                        @can('delivery_person_edit')
                                            ...
                                            @endif
                                            @if(Auth::user()->load('roles')->roles->contains('title', 'vehicle'))
                                                <a href="{{ url('vehicle/deliveryPerson/'.$delivery_person->id.'/edit') }}" class="btn btn-primary btn-action mr-1" data-toggle="tooltip" title="" data-original-title="{{__('Edit')}}"><i class="fas fa-pencil-alt"></i></a>
                                            @endif
                                        @endcan
                                        @if(Auth::user()->load('roles')->roles->contains('title', 'admin'))
                                            <a href="{{ url('admin/delivery_person/'.$delivery_person->id) }}" class="btn btn-primary btn-action mr-1" data-toggle="tooltip" title="" data-original-title="{{__('show Fleet operator')}}"><i class="fas fa-eye"></i></a>
                                        @endif
                                        @if(Auth::user()->load('roles')->roles->contains('title', 'vehicle'))
                                            <a href="{{ url('vehicle/deliveryPerson/'.$delivery_person->id) }}" class="btn btn-primary btn-action mr-1" data-toggle="tooltip" title="" data-original-title="{{__('show Fleet operator')}}"><i class="fas fa-eye"></i></a>
                                        @endif
                                        @can('delivery_person_edit')
                                            <a href="javascript:void(0);" class="table-action ml-2 btn btn-danger btn-action" onclick="deleteData('admin/delivery_person',{{ $delivery_person->id }},'Fleet operator')">
