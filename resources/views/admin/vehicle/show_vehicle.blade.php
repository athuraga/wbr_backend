...
<section class="section">
            ...
            <div class="dropdown d-inline mr-2">
                ...
                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-start"
                    ...
                    <a class="dropdown-item"
                        href="{{ url('admin/vehicle/'.$vehicle->id.'/edit') }}">{{__('Edit Vehicle')}}</a>
                    @endcan
                    @can('admin_vehicle_discount_access')
                    <a class="dropdown-item"
                        href="{{ url('admin/vehicle_discount/'.$vehicle->id)}}">{{__('Discount')}}</a>
                    @endcan
                    @can('admin_vehicle_financeDetails')
                    <a class="dropdown-item"
                        href="{{ url('admin/finance_details/'.$vehicle->id) }}">{{__('finance details')}}</a>
                    @endcan
                    @can('admin_vehicle_deliveryTimeslots')
                    <a class="dropdown-item"
                        href="{{ url('admin/edit_delivery_time/'.$vehicle->id) }}">{{__('Edit delivery time')}}</a>
                    @endcan
                    @can('admin_vehicle_pickupTimeslots')
                    <a class="dropdown-item"
                        href="{{ url('admin/edit_pick_up_time/'.$vehicle->id) }}">{{__('pick up time')}}</a>
                    @endcan
                    @can('admin_vehicle_sellingTimeslots')
                    <a class="dropdown-item" href="{{ url('admin/edit_selling_timeslot/'.$vehicle->id) }}">{{__('selling timeslots')}}</a>
                    @endcan
                    @can('admin_vehicle_reviews')
                    <a class="dropdown-item" href="{{ url('admin/rattings/'.$vehicle->id) }}">{{__('review and rattings')}}</a>
                    @endcan
                    @can('admin_vehicle_bankDetails')
                    <a class="dropdown-item"
                        href="{{ url('admin/vehicle_bank_details/'.$vehicle->id) }}">{{__('add bank details')}}</a>
                    @endcan
                    @can('admin_vehicle_password')
                    <a class="dropdown-item"
                        href="{{ url('admin/vehicle_change_password/'.$vehicle->id) }}">{{__('change password')}}</a>
                    @endcan
                </div>
            </div>
        </div>
    ...
    <div class="section-body">
        ...
        <p class="section-lead">{{__('Information about vehicle')}}</p>
        <div class="row mt-sm-4">
            <div class="col-12 col-md-12 col-lg-12">
                <div class="card profile-widget">
                    <div class="profile-widget-header">
                        <img alt="image" src="{{ $vehicle->image }}" class="rounded-circle profile-widget-picture" width="200px" height="100px">
                        <div class="profile-widget-items">
                            <div class="profile-widget-item">
                                <div class="profile-widget-item-label">{{__('total order')}}</div>
                                <div class="profile-widget-item-value">{{ App\Models\Order::where('vehicle_id',$vehicle->id)->count() }}</div>
                            </div>
                            <div class="profile-widget-item">
                                <div class="profile-widget-item-label">{{__('Pending orders')}}</div>
                                <div class="profile-widget-item-value">{{ App\Models\Order::where([['vehicle_id',$vehicle->id],['order_status','PENDING']])->count() }}</div>
                            </div>
                            <div class="profile-widget-item">
                                <div class="profile-widget-item-label">{{__('Total review')}}</div>
                                <div class="profile-widget-item-value">{{ $vehicle->review }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="profile-widget-description">
                        <div class="profile-widget-name"> {{ $vehicle->email_id }} <div
