...
<head>
    ...
    <title>{{ $title }} | @yield('title','Vehicle login')</title>
            ...
            <div class="d-flex flex-wrap align-items-stretch">
                <div class="col-lg-8 col-12 col-md-6 order-lg-1 order-1 min-vh-100 background-walk-y overlay-gradient-bottom" data-background="{{ url('images/1.png') }}" style="background-color: #23110f">
                    <div class="absolute-bottom-left index-2">
                        <div class="text-light p-5 pb-2">
                            <div class="mb-5 pb-3">
                                <h1 class="mb-2 display-4 font-weight-bold">{{__("welcome Vehicle...!!")}}</h1>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-12 pt-5 col-md-6 order-lg-2 min-vh-100 order-2 bg-white">
                    <div class="p-4 m-3">
                        ...
                        <h4 class="text-dark mb-5 font-weight-normal">{{__('Welcome to ')}}<span class="font-weight-bold">{{__('Vehicle')}}</span>
                        </h4>
                        @if ($errors->any())
                        ...
                        @endif
                        <form method="POST" action="{{ url('vehicle/vehicle_confirm_login') }}">
                            @csrf
                        ...
                        <div class="text-center">
                            <a href="{{ url('vehicle/forgot_password') }}" class="text-small mt-2">
                                {{__('Forgot Password?')}}
                            </a>
                        </div>
                        <div class="text-muted text-center mt-2">
                            {{__("Don't have an account?")}}
                            <a href="{{ url('vehicle/register_vehicle') }}">{{__('Register')}}</a>
                        </div>
                    </div>
                </div>
            </div>
