...
function update_area()
{
    ...
    $.ajax(
    {
        ...
        data:
        {
            ...
            delivery_zone_id : $('#delivery_zone_id').val(),
            vehicle_id : $('#edit_vehicle_id').val(),
        },
        success: function (result)
        {
            if(result.success == true)
    ...
    $.ajax(
    {
        ...
        success: function (result)
        {
            if(result.success == true)
            {
                ...
                if(result.data.vehicle_id.length > 0){
                    var vehicle = result.data.vehicle_id.split(',');
                    $("#edit_vehicle_id").val("");
                    $("#edit_vehicle_id").trigger("change");

                    vehicle.forEach(element =>
                    {
                        $('select[id="edit_vehicle_id"] option[value='+element+']').attr("selected",true);
                        $('select[id="edit_vehicle_id"] option[value='+element+']').trigger('change');
                    });
                }
