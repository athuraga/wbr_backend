...
$(document).ready(function() {
    ...
    if (window.location.origin + window.location.pathname == $('#mainurl').val() + '/vehicle/vehicle_home') {
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
...
function show_settle_details(index) {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success == true) {
                $('.details_body').html('');
                $('.details_body').append('<div class="table-responsive"><table class="table"><thead class="display"><tr><th>Date</th><th>Vehicle earning</th><th>Settlement with</th><th>Payment token</th></tr></thead></table></div>');
                if (result.data.length != 0) {
                    result.data.forEach(element => {
                        if (element.vehicle_status == 1) {
                            var token = element.payment_token == null ? "Payment with cod" : element.payment_token;
                            $('.display').after('<tbody><td>' + element.date + '</td><td>' + result.currency + element.vehicle_earning + '</td><td>' + element.payment_type + '</td><td>' + token + '</td></tbody>');
                        } else {
                            $('.display').after('<tbody><td>' + element.date + '</td><td>' + result.currency + element.vehicle_earning + '</td><td class="text-center" colspan=2>' + "Payment not complete" + '</td></tbody>');
                        }
                    });
                } else {
...
function update_vehicletype(id) {
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result.success == true) {
                $('#update_image').attr('src', result.data.image);
                $('#update_vehicletype_form').attr("action", base_url + "/admin/vehicletype/" + result.data.id);
