...
$(document).ready(function() {
    ...
    $(".show_vehicle_model_select2").select2({
        width: "100%",
        height: "100%",
    ...
    if (
        window.location.origin + window.location.pathname ==
        $("#mainurl").val() + "/vehicle/vehicle_home"
    ) {
        $.ajax({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            ...
            type: "GET",
            url: base_url + "/vehicle/revenueChart",
            success: function(result) {
                console.log("result", result);
                revenueChart(result);
            },
        ...
        $.ajax({
            ...
            type: "GET",
            url: base_url + "/vehicle/orderChart",
            success: function(result) {
                console.log("result", result);
                vehicle_userChart(result);
            },
            error: function(err) {
                console.log("err ", err);
            },
        ...
        $.ajax({
            ...
            type: "GET",
            url: base_url + "/vehicle/vehicleAvarageTime",
            success: function(result) {
                avarageVehicleItems(result);
            },
            error: function(err) {
                console.log("err ", err);
            },
    ...
    $("select[name=submenu_filter]").change(function() {
        ...
        $.ajax({
            ...
            url: base_url + "/vehicle/vehicle_menu/" + menu_id,
            success: function(result) {
    ...
    $("input[type=checkbox][name=vehicle_notification]").change(function() {
        if (this.checked == true) {
            $(".vehicle_notification_card").show();
        } else {
            $(".vehicle_notification_card").hide();
            $.ajax({
                headers: {
                    "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                        "content"
...
function vehicle_userChart(data) {
...
function avarageVehicleItems(data) {
    var ctx = document.getElementById("vehicleAvarageTime").getContext("2d");
    var myChart = new Chart(ctx, {
        type: "doughnut",
        // data:
        // {
...
function VehicleorderChart(data) {
    color = [];
    for (let i = 0; i < 7; i++) {
        color.push(
            "rgb(" +
...
function update_custimization(i, name) {
    ...
    $("#custom" + i).append(
        ...
        name +
        '[]" class="form-control" required placeholder="item" style="text-transform: none;"></td><td><input type="number" name="price[]" class="form-control" required placeholder="price"></td><td><input type="radio" name="isdefault_' +
        name +
        '" value="' +
        tds +
        '" id="' +
...
function add_field() {
    $(".delivery_table").append(
        '<tr><td><input type="number" required name="min_value[]" class="form-control"></td><td><input type="number" required name="max_value[]" class="form-control"></td><td><input type="number" required name="charges[]" class="form-control"></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}
...
function add_driverearning_field() {
    $(".driver_earning_table").append(
        '<tr><td><input type="number" name="min_km[]" class="form-control" required></td><td><input type="number" name="max_km[]" class="form-control" required></td><td><input type="number" name="charge[]" class="form-control" required></td><td><button type="button" class="btn btn-primary removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}
...
//         },
//         type: "GET",
//         url: base_url + '/vehicle/order/view_custimization/' + submenu_id,
//         success: function (result) {
//             console.log(result);
//             if (result.success == true) {
...
//         },
//         type: "POST",
//         url: base_url + '/vehicle/order/update_custimization',
//         data:
//         {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success == true) {
                ...
                $(".show_vehicle_amount").text(
                    result.data.currency + result.data.order.vehicle_amount
                );
                $(".TaxTable").html("");
                ...
                } else if (result.data.order.vehicle_discount_price != null) {
                    $(".show_discount").text(
                        result.data.currency +
                        result.data.order.vehicle_discount_price
                    );
                } else {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success == true) {
                ...
                // $('.show_vehicle_amount').text(result.data.currency + result.data.order.vehicle_amount);
                // $('.show_order_table').html('');
                // result.data.order.orderItems.forEach(element => {
                ...
                // }
                // else if (result.data.order.vehicle_discount_price != null) {
                //     $('.show_discount').text(result.data.currency + result.data.order.vehicle_discount_price);
                // }
                // else {
                //     $('.show_discount').text("doesn't apply any promo code");
                // }
...
//         },
//         type: "POST",
//         url: base_url + '/vehicle/order/view_total',
//         data:
//         {
//             id: id,
//         },
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success == true) {
                $(".details_body").html("");
                $(".details_body").append(
                    '<div class="table-responsive"><table class="table"><thead class="display"><tr><th>Date</th><th>Vehicle earning</th><th>Settlement with</th><th>Payment token</th></tr></thead></table></div>'
                );
                if (result.data.length != 0) {
                    result.data.forEach((element) => {
                        if (element.vehicle_status == 1) {
                            var token =
        ...
        success: function(result) {
            if (result.success == true) {
                ...
                $(".edit_notification_template_form").attr(
                    "action",
                    base_url + "/admin/notification_template/" + result.data.id
                );
            }
        },
        error: function(err) {
...
//         },
//         type: "POST",
//         url: base_url + '/vehicle/order/removeSingleItem',
//         data:
//         {
//             id: id,
//         },
...
function order_status(id) {
    $.ajax({
        ...
        url: base_url + "/vehicle/order/change_status",
        success: function(result) {
            if (result.success == true) {
                location.reload();
...
function update_submenucustimization(id) {
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result.data.qty_reset == "scooter") {
                $("#scooter").prop("checked", true);
            } else {
...
function update_vehicletype(id) {
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        ...
        url: base_url + "/admin/vehicletype/" + id + "/edit",
        success: function(result) {
            console.log("result", result);
            if (result.success == true) {
                $("#update_image").attr("src", result.data.image);
                $("#update_vehicletype_form").attr(
                    "action",
                    base_url + "/admin/vehicletype/" + result.data.id
                );
                $("#update_vehicletype").val(result.data.name);
            }
        },
        error: function(err) {
            console.log("err ", err);
...
function driver_assign(order_id) {
    $.ajax({
        ...
        url: base_url + "/vehicle/order/driver_assign",
        success: function(result) {
            if (result.success == true) {
