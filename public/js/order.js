...
function change_submenu(menu_id) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: base_url + "/vehicle/change_submenu",
        data: {
            menu_id: menu_id,
        },
        success: function(result) {
            ...
            for (let i = 0; i < result.data.length; i++) {
                ...
                var cust;
                var icon;
                if (result.data[i].type == "scooter") {
                    icon =
                        '<img src="' +
                        base_url +
                        '/images/scooter.png" alt="" class="orderIcon">';
                } else {
                    icon =
                        '<img src="' +
                        base_url +
                        '/images/bike.png" alt="" class="orderIcon">';
                }
                ...
                $(".orderMainRow").append(
                    '<div class="col-sm-12 col-md-12 col-lg-12 col-xl-6 orderMainCol"><div class="orderCard"><div class="orderCardBody"><div class="row"><div class="col-sm-12 col-md-12 col-lg-12 col-xl-4 orderCol"><img src="' +
                    result.data[i].image +
                    '" width="100%" height="113" alt="" class="orderImage"></div><div class="col-sm-12 col-md-12 col-lg-12 col-xl-8 orderColRight"><div class="orderContent">' +
                    icon +
                    "<h5>" +
                    result.data[i].name +
                    '</h5></div><span class="text-muted orderDesc" id="orderContent' +
                    result.data[i].id +
                    '">' +
                    result.data[i].description +
                    '</span><br><a onclick="DispCustimization(' +
                    result.data[i].id +
                    ')" class="text-primary cursor-pointer ' +
                    cust +
                    " custimization" +
                    result.data[i].id +
                    '">Custimization</a><div class="orderPriceQty" id=orderQty' +
                    result.data[i].id +
...
function showUser() {
    ...
    for (let i = 0; i < table_length; i++) {
        if (
            $("input[type=checkbox][id=custchkbox" + i + "]:checked").val() !=
            undefined
        ) {
            ...
            for (let j = 0; j < radio_length; j++) {
                if (
                    $(
                        "input[type=radio][id=radio" + i + "_" + j + "]:checked"
                    ).val() != undefined
                ) {
                    var temp = $(
                        "input[type=radio][id=radio" + i + "_" + j + "]:checked"
            ...
            custimization.push({
                main_menu: $(
                    "input[type=checkbox][id=custchkbox" + i + "]:checked"
                ).val(),
                data: {
                    name: name,
                    price: price,
    ...
    $.ajax({
        ...
        type: "post",
        url: base_url + "/vehicle/update_custimization",
        data: {
...
function displayBill() {
    ...
    if (user != undefined) {
        ...
        $.ajax({
            ...
            type: "GET",
            url: base_url + "/vehicle/display_bill",
            success: function(result) {
                if (result.success == true) {
                    ...
                    $("input[name=hidden_all_tax]").val(
                        JSON.stringify(result.data.admin_tax)
                    );
                    ...
                    result.data.admin_tax.forEach((element) => {
                        $("#taxCharge").after(
                            '<tr class="Border dispBill"><td class="leftTd">' +
                            element.name +
                            '</td> <td class="rightTd">' +
                            result.currency +
                            '<span class="dispBillTex">' +
                            element.tax +
                            "</span></td></tr>"
                        );
                    });
...
function displayBillWithCoupen(id) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: base_url + "/vehicle/displayBillWithCoupen",
        data: {
            promo_id: id,
            user_id: user_id,
            tax: tax,
        ...
        success: function(result) {
            if (result.success == true) {
                ...
                $("input[name=hidden_promocode_price]").val(
                    result.data.discount
                );
                $("input[name=hidden_promocode_id]").val(
                    result.data.procode_id
                );
...
function confirm_order() {
    $.ajax({
        ...
        url: base_url + "/vehicle/order",
        success: function(result) {
            if (result.success == true) {
                location.replace(base_url + "/vehicle/Orders");
            }
        },
        error: function(err) {
            console.log("err ", err);
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success == true) {
                ...
                $("#orderQty" + id).append(
                    '<span class="orderQty">' + result.data.qty + "</span>"
                );
...
function DispCustimization(submenu_id) {
    $.ajax({
        ...
        type: "GET",
        url: base_url + "/vehicle/DispCustimization/" + submenu_id,
        success: function(result) {
            if (result.success == true) {
                ...
                if (result.data.item.length > 0) {
                    $.each(result.data.item, function(i, element) {
                        ...
                        $(".displayCustimization").append(
                            '<table class="table custimization_table"><tr><td class="cust_items' +
                            i +
                            '"><hr><input type="checkbox" value=' +
                            element.name +
                            ' id="custchkbox' +
                            i +
                            '" name="custimization' +
                            element.name +
                            '"><label for="custchkbox' +
                            i +
                            '">' +
                            element.name +
                            "</label><hr></td></tr></table>"
                        );
                        if (element.custimazation_item != null) {
                            ...
                            $.each(c, function(j, item) {
                                ...
                                $(".cust_items" + i).append(
                                    ...
                                    '">' +
                                    item.name +
                                    '</label></td><td class="text-right w-100"><label for="radio' +
                                    j +
                                    '">' +
                                    result.currency +
                                    item.price +
                                    ...
                                    if (result.data.session != "") {
                                        ...
                                        $.each(s, function(k, session) {
                                            if (
                                                session.main_menu ==
                                                $(
                                                    'input[type=checkbox][name="custimization' +
                                                    element.name +
                                                    '"]'
                                                ).val()
                                            ) {
                            ...
                            $(".displayCustimization").append(
                                "No custimization available"
                            );
                        }
                    });
                } else {
                    $(".displayCustimization").append(
                        '<h5 class="text-center">No custimization available</h5>'
                    );
                }
            }
        },
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result.success == true) {
                ...
                $(".add_user").removeClass("cardBottom");
                $(".displayUser").append(
                    '<li class="media p-3 single_record"><img alt="image" width="50" src=' +
                    result.data.image +
                    ' class="mr-3 rounded-circle"> <div class="media-body"><div class="media-title">' +
                    result.data.name +
                    '</div> <div class="text-job text-muted">' +
                    result.data.email_id +
                    "<br></div>" +
                    result.data.phone +
                    '</div> <div class="media-item"><div class="media-value"><input type="radio" value=' +
                    result.data.id +
                    ' id="chkbox' +
                    result.data.id +
                    '" name="user"></div></div></li>'
                );
            }
        },
        error: function(err) {
