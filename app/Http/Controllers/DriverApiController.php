...
class DriverApiController extends Controller
{
    ...
    {
        $vehicle_driver = $this->checkVehicleDriver();
        if($vehicle_driver == 1)
        {
    ...
    public function apiSetLocation(Request $request)
    {
        ...
        {
            return response(['success' => false , 'data' => 'vehicle driver']);
        }
    }
    ...
    public function apiDriverOrder()
    {
        ...
        if($vehicle_driver == 1) 
        {
            ...
            {
                $a = explode(',',$delivery_zone_area->vehicle_id);
                foreach ($a as $value)
                {
                    if($value)
                    {
                        $near_vehicles = Vehicle::find($value)->GetByDistance($delivery_zone_area->lat, $delivery_zone_area->lang, $delivery_zone_area->radius)->get();
                        if ($near_vehicles != null)
                        {
                            foreach ($near_vehicles as $near_vehicle)
                            {
                                array_push($vehicles,$near_vehicle->id);
                            }
                        }
                    }
            ...
            $orders = Order::whereIn('user_id',$users)->whereIn('vehicle_id',$vehicles)->where('delivery_type','HOME')->where('order_status','APPROVE')->get(['id','address_id','order_id','user_id','vehicle_id','payment_status','address_id','order_status','amount','payment_type']);
            return response(['success' => true , 'data' => $orders]);
        }
        else
        {
            $orders = Order::where([['delivery_person_id',$driver->id],['order_status','APPROVE']])->get(['id','address_id','order_id','user_id','payment_status','address_id','order_status','vehicle_id','amount','payment_type']);
            return response(['success' => true , 'data' => $orders]);
        }
    }
        ...
        if($order)
        {
            ...
            if($vehicle_driver == 1)
            {
                ...
                if($reqData['order_status'] == 'COMPLETE')
                {
                    ...
                    $distance = $this->distance($vehicle->lat,$vehicle->lang,$order->user_address['lat'],$order->user_address['lang'],'K');
    ...
    public function apiOrderHistory()
    {
        app('App\Http\Controllers\Vehicle\VehicleSettingController')->cancel_max_order();
        $this->driver_cancel_max_order();
        $orders['complete'] = Order::where([['delivery_person_id',auth()->user()->id],['order_status','COMPLETE']])->get(['id','address_id','amount','order_id','user_id','vehicle_id','payment_status','order_status']);
        $orders['cancel'] = Order::where([['delivery_person_id',auth()->user()->id],['order_status','CANCEL']])->get(['id','address_id','amount','order_id','cancel_reason','user_id','vehicle_id','payment_status','order_status']);
        return response(['success' => true , 'data' => $orders]);
    }
    ...
    public function apiEarningHistory()
    {
        ...
        if ($vehicle_driver == 1)
        {
            ...
            for ($i = 0; $i < 12; $i++)
            {
                ...
                {
                    if (isset($graph->vehicle['lat']) && isset($graph->vehicle['lang']) && isset($graph->user_address['lat']) && isset($graph->user_address['lang']))
                    {
                        $distance = $this->distance($graph->vehicle['lat'],$graph->vehicle['lang'],$graph->user_address['lat'],$graph->user_address['lang']);
    ...
    public function apiDriverSetting()
    {
        ...
        {
            $vehicle_driver = $this->checkVehicleDriver();
            $setting->global_driver = $vehicle_driver == 1 ? 'true' : 'false';
        }
    ...
    public function checkVehicleDriver()
    {
        // 1 for global driver , 0 for vehicle driver
        ...
        if ($LoginDriver) {
            return $LoginDriver->vehicle_id == null ? 1 : 0;
        }
    }
}