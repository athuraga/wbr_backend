...
class UserApiController extends Controller
{
    ...
    public function apiSingleVehicle($vehicle_id)
    {
        ...
        $master['menu'] = $menus;
        $master['vehicle_discount'] = VehicleDiscount::where('vehicle_id', $vehicle_id)->orderBy('id', 'desc')->first(['id', 'type', 'discount', 'min_item_amount', 'max_discount_amount', 'start_end_date']);
        $master['delivery_timeslot'] = WorkingHours::where([['type', 'delivery_time'], ['vehicle_id', $vehicle_id]])->get(['id', 'day_index', 'period_list', 'status']);
        $master['pick_up_timeslot'] = WorkingHours::where([['type', 'pick_up_time'], ['vehicle_id', $vehicle_id]])->get(['id', 'day_index', 'period_list', 'status']);
        $master['selling_timeslot'] = WorkingHours::where([['type', 'selling_timeslot'], ['vehicle_id', $vehicle_id]])->get(['id', 'day_index', 'period_list', 'status']);
    ...
    public function apiPromoCode($vehicle_id)
    {
    ...
    public function apiVehicletype()
    {
        $vehicletypes = Vehicletype::where('status', 1)->orderBy('id', 'DESC')->get();
        return response(['success' => true, 'data' => $vehicletypes]);
    }
    ...
    public function apiBookOrder(Request $request)
    {
        ...
        $vehicle = Vehicle::where('id', $bookData['vehicle_id'])->first();
        ...
        $bookData['vehicle_id'] = $vehicle->id;
        ...
        $this->sendVehicleOrderNotification($vehicle, $bookData['order_id']);
        $this->sendUserNotification($bookData['user_id'], $bookData['order_id']);
        $amount = $order->amount;
        $tax = array();
        if ($vehicle->admin_comission_type == 'percentage') {
            $comm = $amount * $vehicle->admin_comission_value;
            $tax['admin_commission'] = intval($comm / 100);
            $tax['vehicle_amount'] = intval($amount - $tax['admin_commission']);
        }
        if ($vehicle->admin_comission_type == 'amount') {
            $tax['vehicle_amount'] = $amount - $vehicle->admin_comission_value;
            $tax['admin_commission'] = $amount - $tax['vehicle_amount'];
        }
        $order->update($tax);
    ...
    {
        app('App\Http\Controllers\Vehicle\VehicleSettingController')->cancel_max_order();
        // app('App\Http\Controllers\DriverApiController')->cancel_max_order();
        $orders = Order::where('user_id', auth()->user()->id)->orderBy('id', 'DESC')->get(['id', 'amount', 'vehicle_id', 'order_status', 'delivery_person_id', 'delivery_charge', 'date', 'time', 'address_id']);
    ...
    public function apiVehicle(Request $request)
    {
        return Vehicle::get();
        $vehicles = Vehicle::where('status', 1)->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        foreach ($vehicles as $vehicle) {
            ...
            $vehicle['distance'] = round($distance);
            if (auth('api')->user() != null) {
                $user = auth('api')->user();
                $vehicle['like'] = in_array($vehicle->id, explode(',', $user->faviroute));
            } else {
                $vehicle['like'] = false;
            }
        }
    ...
    public function apiNearBy(Request $request)
    {
        ...
        $vehicles = Vehicle::where('status', 1)->GetByDistance($request->lat, $request->lang, $radius)->get(['id', 'image', 'name', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        foreach ($vehicles as $vehicle) {
            if (auth('api')->user() != null) {
                $user = auth('api')->user();
                $vehicle['like'] = in_array($vehicle->id, explode(',', $user->faviroute));
    ...
    public function apiRestFaviroute(Request $request)
    {
        ...
        $vehicles = Vehicle::where('status', 1)->whereIn('id', $faviroute)->get(['id', 'name', 'image', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        foreach ($vehicles as $vehicle) {
            $lat1 = $vehicle->lat;
            $lon1 = $vehicle->lang;
            $lat2 = $request->lat;
    ...
    {
        $result = Vehicle::where('status', 1);
        $v = [];
        if (isset($request->cousins)) {
            $vehicles = Vehicle::where('status', 1)->get();
            foreach ($vehicles as $vehicle) {
                $vehicletypeId = explode(',', $vehicle->vehicletype_id);
                if (($key = array_search($request->cousins, $vehicletypeId)) !== false) {
                    array_push($v, $vehicle->id);
                }
            }
        ...
        if (isset($request->quick_filter)) {
            $result = $result->where('vehicle_type', $request->quick_filter);
        }
    ...
    {
        $vehicles = Vehicle::where([['isTop', '1'], ['status', 1]])->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'vehicle_type', 'vehicletype_id'])->makeHidden(['vehicle_logo']);
        foreach ($vehicles as $vehicle) {
            $lat1 = $vehicle->lat;
            $lon1 = $vehicle->lang;
            $lat2 = $request->lat;
    ...
    {
        $vehicles = Vehicle::where([['isExplorer', '1'], ['status', 1]])->orderBy('id', 'DESC')->get(['id', 'image', 'name', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        foreach ($vehicles as $vehicle) {
            $lat1 = $vehicle->lat;
            $lon1 = $vehicle->lang;
            $lat2 = $request->lat;
    ...
    {
        $order = Order::where('id', $id)->first(['id', 'order_id', 'vehicle_id', 'amount', 'delivery_person_id', 'order_status', 'address_id', 'promocode_id', 'promocode_price', 'user_id', 'vehicle_discount_price', 'delivery_charge']);
        $tax = 0;
    ...
    public function apiVehicletypeVehicle($id)
    {
        $v = Vehicle::where('status', 1)->get(['id', 'image', 'name', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        $vehicles = array();
        foreach ($v as $vehicle) {
            $vehicletypes = explode(',', $vehicle->vehicletype_id);
            if (($key = array_search($id, $vehicletypes)) !== false) {
    ...
    public function apiSearch(Request $request)
    {
        ...
        $req = [];
        $radius = GeneralSetting::first()->radius;
        $req['vehicle'] = Vehicle::where('status', 1)->get(['id', 'image', 'name', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        // $req['vehicle'] = Vehicle::where('status', 1)->GetByDistance($request->lat, $request->lang, $radius)->get(['id', 'image', 'name', 'lat', 'lang', 'vehicletype_id', 'vehicle_type'])->makeHidden(['vehicle_logo']);
        // $req['vehicle'] = Vehicle::where('name','LIKE','%'.$data['name']."%")->GetByDistance($request->lat, $request->lang, $radius)->get(['id','image','name','lat','lang','vehicletype_id','vehicle_type']);
        $req['vehicletype'] = Vehicletype::where('name', 'LIKE', '%' . $data['name'] . "%")->get(['id', 'name', 'image']);
    ...
    public function apiTracking($order_id)
    {
        ...
        $temp['vehicle_lat'] = Vehicle::find($order->vehicle_id)->lat;
        $temp['vehicle_lang'] = Vehicle::find($order->vehicle_id)->lang;
    ...
    public function apiAddReview(Request $request)
    {
        ...
        if (Review::where([['order_id', $data['order_id'], ['user_id', auth()->user()->id]]])->exists() != true) {
            ...
            $data['vehicle_id'] = Order::find($data['order_id'])->vehicle_id;
            $d_image = [];
            if (isset($data['image'])) {
    ...
    public function apiUserOrderStatus()
    {
        ...
        // $order = Order::where('user_id',$user)->where('order_status','!=','COMPLETE')->where('order_status','!=','CANCEL')->get(['order_status','id'])->makeHidden(['vehicle','user','orderItems','user_address']);
        $order = Order::where('user_id', $user->id)->where([['order_status', '!=', 'COMPLETE'], ['order_status', '!=', 'PENDING'], ['order_status', '!=', 'CANCEL']])->get(['order_status', 'id'])->makeHidden(['vehicle', 'user', 'orderItems', 'user_address']);
    ...
    public function sendVehicleOrderNotification($vehicle, $order_id)
    {
        $vehicle_notification = GeneralSetting::first()->vehicle_notification;
        $vehicle_mail = GeneralSetting::first()->vehicle_mail;
        $content = NotificationTemplate::where('title', 'vehicle order')->first();
        $vehicle_user = User::where('id', $vehicle->user_id)->first();
        if ($vehicle->vehicle_language == 'spanish') {
            $detail['Vehicle_name'] = $vehicle->name;
            $detail['Order_id'] = $order_id;
            $detail['User_name'] = auth()->user()->name;
            $v = ["{Vehicle_name}", "{Order_id}", "{User_name}"];
            $notification_content = str_replace($v, $detail, $content->spanish_notification_content);
            if ($vehicle_notification == 1) {
                try {
                    Config::set('onesignal.app_id', env('vehicle_app_id'));
                    Config::set('onesignal.rest_api_key', env('vehicle_api_key'));
                    Config::set('onesignal.user_auth_key', env('vehicle_auth_key'));
                    OneSignal::sendNotificationToUser(
                        $notification_content,
                        $vehicle_user->device_token,
                        $url = null,
                        $data = null,
                        $buttons = null,
                        $schedule = null,
            ...
            $p_notification['user_type'] = 'vehicle';
            $p_notification['user_id'] = $vehicle->id;
            ...
            if ($vehicle_mail == 1) {
                try {
                    Mail::to($vehicle->email_id)->send(new VehicleOrder($mail));
                } catch (\Throwable $th) {
                }
            }
            return true;
    ...
    public function apiUserBalance()
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            if ($transaction->type == 'withdraw') {
                $transaction->order = Order::find($transaction->meta[0], ['id', 'vehicle_id', 'order_id'])->makeHidden(['user_address', 'orderItems', 'user', 'vehicle']);
                $transaction->vehicle_name = vehicle::find($transaction->order->vehicle_id)->name;
            }
        }
