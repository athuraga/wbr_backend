...
class ReportController extends Controller
{
    ...
    public function vehicle_report(Request $request)
    {
        ...
        {
            $date = explode(' - ',$request->date_range);
            $vehicles = Vehicle::whereBetween('created_at', [$date[0].' 00:00', $date[1].' 23:59'])->get();
        }
        else
        {
        ...
        {
            $vehicle['total_order'] = Order::where('vehicle_id',$vehicle->id)->count();
            $vehicle['vehicle_earning'] = Settle::where('vehicle_id',$vehicle->id)->sum('vehicle_earning');
            $vehicle['remain_settle'] = Settle::where([['vehicle_id',$vehicle->id],['vehicle_status',0]])->sum('vehicle_earning');
            $vehicle['compelte_settle'] = Settle::where([['vehicle_id',$vehicle->id],['vehicle_status',1]])->sum('vehicle_earning');
        }
        $currency = GeneralSetting::first()->currency_symbol;
        return view('admin.report.vehicle_report',compact('vehicles','currency'));
    }
